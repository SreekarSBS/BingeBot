...
const SignUp = () => {
     const [showPassword,setShowPassword] = useState(false);
        ...
        //    validateData( email ?? "" ,password?.current?.value ?? "" )
    ...
    return (
        <div className="bg-white w-full h-screen text-black ">
            <div className="text-3xl m-16 p-16 w-1/2 h-96  mx-auto my-auto fixed top-0 bottom-0 left-0 right-0">
                ...
                <input  type = "email"  defaultValue={`${email}`}  className=" bg-slate-100 xl:w-full mt-6  rounded-lg xl:h-8 2xl:h-10 my-4 p-2 text-gray-600  "  />
                <div className="flex items-center xl:w-full   ">
                <input ref = {password} type = {`${showPassword ? "text" : "password" }`} placeholder="Password" className="bg-slate-100  w-full rounded-lg xl:h-8 2xl:h-10 my-4 p-2 text-gray-500 " />
